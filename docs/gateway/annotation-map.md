...
| 2 | `cert-manager.io/cluster-issuer: <x>` | portable | **migrate to Gateway annotation** — add `cert-manager.io/cluster-issuer: <x>` to `Gateway.metadata.annotations`. cert-manager v1.15+ gateway-shim reads this + `listener.tls.certificateRefs[name]` and auto-creates/renews the Certificate CR. No explicit `Certificate` resource needed. Requires cert-manager ≥1.15 with gateway-shim enabled. | Same as Traefik path when `spec.tls[].secretName` is present. If the source also had `networking.gke.io/managed-certificates`, that takes precedence for GKE. | move annotation from Ingress → Gateway |
